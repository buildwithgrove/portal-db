...
var (
	...
	testLegacyLoadBalancer = LoadBalancer{
		...
		UserID:            "test_user_5736487d3c1975",
		...
		Users: []UserAccess{
			{UserID: "test_user_5736487d3c1975", RoleName: RoleOwner, Email: "test_owner@user.com", Accepted: true},
			{UserID: "test_user_4d89ede3e3135f", RoleName: RoleMember, Email: "test_member@user.com", Accepted: true},
		},
	...
	testLegacyApplication = Application{
		...
		Limit: AppLimit{
			Plan:        PayPlan{Type: FreetierV0, Limit: 250_000},
			CustomLimit: 0,
		},
		...
		Notifications: []UpdateAppNotifications{
			{
				...
				Events: []NotificationEvent{
					NotificationEventSignedUp,
					NotificationEventQuarter,
					NotificationEventThreeQuarters,
				},
			},
		},
	...
	testV2AccountUserAccess = AccountUserAccess{
		UserID:          0, // autogenerated by DB
		Email:           "test_admin@user.com",
		RoleName:        RoleAdmin,
		Accepted:        true,
		ProviderUserIDs: map[AuthType]string{AuthTypeAuth0Username: "test_user_c66f399519ba23"},
	}
)
...
func Test_LegacyAdapators_ConvertToLegacyPayPlan(t *testing.T) {
	...
	tests := []struct {
		name                  string
		plan                  Plan
		expectedLegacyPayPlan PayPlan
	}{
		{
			name:                  "Should convert a V2 Plan struct to a legacy PayPlan struct",
			plan:                  testPortalApplication.Account.Plan,
			expectedLegacyPayPlan: testLegacyPayPlan,
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			legacyPayPlan := test.plan.ConvertToLegacyPayPlan()
...
func Test_LegacyAdapators_ConvertToV2UpdateAccountUserAccess(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			v2AccountUserAccess := test.userAccess.ConvertToV2UpdateAccountUserAccess(test.userID, test.accepted)
			test.expectedV2AccountUserAccess.UserID = UserID(test.userID)
			c.Equal(test.expectedV2AccountUserAccess, v2AccountUserAccess)
		})
	}
}
