...
const selectAccountAppAccess = `-- name: SelectAccountAppAccess :many
SELECT aua.user_id,
    aua.account_id,
...
type SelectAccountAppAccessRow struct {
	UserID              types.UserID      `json:"user_id"`
	AccountID           types.AccountID   `json:"account_id"`
	PortalApplicationID types.PortalAppID `json:"portal_application_id"`
	RoleName            types.RoleName    `json:"role_name"`
...
func (q *Queries) SelectAccountAppAccess(ctx context.Context) ([]SelectAccountAppAccessRow, error) {
	rows, err := q.db.Query(ctx, selectAccountAppAccess)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SelectAccountAppAccessRow
	for rows.Next() {
		var i SelectAccountAppAccessRow
		if err := rows.Scan(
			&i.UserID,
			&i.AccountID,
			&i.PortalApplicationID,
