...
const selectPendingAccountInvitations = `-- name: SelectPendingAccountInvitations :many
SELECT aua.user_id, aua.account_id, aua.portal_application_id, aua.role_name
    FROM account_user_access AS aua
WHERE aua.accepted = false
`

type SelectPendingAccountInvitationsRow struct {
	UserID              types.UserID      `json:"user_id"`
	AccountID           types.AccountID   `json:"account_id"`
	PortalApplicationID types.PortalAppID `json:"portal_application_id"`
	RoleName            types.RoleName    `json:"role_name"`
...
func (q *Queries) SelectPendingAccountInvitations(ctx context.Context) ([]SelectPendingAccountInvitationsRow, error) {
	rows, err := q.db.Query(ctx, selectPendingAccountInvitations)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SelectPendingAccountInvitationsRow
	for rows.Next() {
		var i SelectPendingAccountInvitationsRow
		if err := rows.Scan(
			&i.UserID,
			&i.AccountID,
			&i.PortalApplicationID,
